...
![](images/introduccionuno.png){fig-align="center"}
...
Si un archivo CSV no tiene encabezados, especifique la opción encabezado = FALSO como se muestra en el siguiente comando, y R asignará nombres de características predeterminadas en la formaV1, V2:

```{r}
mydata <- read.csv("pt_data.csv", stringsAsFactors = FALSE,
 header = FALSE)
```
...
```{r}
summary(pt_data[c("price", "mileage")])
```
...
```{r}
boxplot(pt_data$price, main="Boxplot of Used Car Prices",
 ylab="Price ($)")
```

```{r}
boxplot(pt_data$mileage, main="Boxplot of Used Car Mileage",
 ylab="Odometer (mi.)")
```
...
```{r}
hist(pt_data$price, main = "Histogram of Used Car Prices",
 xlab = "Price ($)")
```
...
```{r}
hist(pt_data$mileage, main = "Histogram of Used Car Mileage",
 xlab = "Odometer (mi.)")
```
...
```{r}
plot(x = pt_data$mileage, y = pt_data$price,
...
```{r}
pt_data$conservative <-
 pt_data$color %in% c("Black", "Gray", "Silver", "White")
```
